...
ENV = {
    ...
    "OPENROUTER_FAST_MODEL": "inception/mercury-2.5", "OPENROUTER_GLM_MODEL": "z-ai/glm-5.3-flash",
    "CODEX_BASE_URL": "http://172.20.142.7:8787", "CODEX_GPT6_MODEL": "gpt-6", "CODEX_PROXY_KEY": "codex-k", "LITELLM_TURN_OFF_MESSAGE_LOGGING": "true",
}
...
def test_render_substitutes_all_placeholders_and_is_valid_yaml():
    ...
    assert names == ["ember-embed", "ember-rerank", "ember-stt", "ember-tts",
                     "local-fast", "local-smart", "local-code", "local-vision", "cloud-fast", "cloud-glm", "cloud-gpt6"]
...
def test_alias_prefixes_match_their_tier():
    ...
    assert by["cloud-gpt6"] == {"model": "openai/gpt-6", "api_base": "http://172.20.142.7:8787/v1", "api_key": "os.environ/CODEX_PROXY_KEY"}
...
def test_non_chat_aliases_declare_their_mode():
    ...
    for chat in ("local-fast", "local-smart", "local-code", "local-vision", "cloud-fast", "cloud-glm", "cloud-gpt6"):
