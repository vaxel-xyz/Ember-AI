...
browser ── chat.vaxel.xyz ── Cloudflare tunnel ──► Docker01 :3003 (open-webui container)
                                              ...
                                              LiteLLM gateway :4000/v1  (ai.vaxel.xyz/v1)
                                                      │
                                                      ▼
                                    oMLX on jons-mac-mini  /  OpenRouter (heavy)
```
...
- **Models:** the picker shows the human-facing aliases `local-fast`, `local-smart` and
  `heavy` (plus the `ember-*` aliases). `local-smart` is the default (`DEFAULT_MODELS`).
