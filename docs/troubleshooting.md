...
| `degraded` | Reachable but not functional — oMLX has `loaded_count == 0`, or LiteLLM readiness is fine but `/model/info` failed or is missing published aliases. | For oMLX: usually harmless, the next request triggers an on-demand load. If it persists after a request, run `~/.omlx/bin/omlx restart` and confirm the model id in `.env` matches a folder under `~/.omlx/models`. For LiteLLM: the `missing` list in `detail` names the aliases the gateway did not register — almost always a render failure or a bad `litellm_params` entry, so read `docker compose logs litellm`. Individual deployments failing does **not** make the gateway `degraded`; use the deep check below for that. |
