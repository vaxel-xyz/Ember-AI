...
## Addendum 6 (2026-09-17, Jon-directed) — Hermes as a distinct Open WebUI backend (ADR 0012)
- Branch: `feature/hermes-openwebui-backend` (stacked on `feature/codex-gpt6` / PR #3). Commits: `a425516c` (feat: ADR 0012 + reference-stack plural-connection config + docs; ci + secret-scan success) + `docs: Hermes backend validation receipt`.
- Per Jon's shared-knowledge ADR §5–7: Hermes exposed to Open WebUI as a **second, separate OpenAI-compatible connection** (model `hermes-agent`) — deliberately NOT routed through LiteLLM, preserving the agent loop, tools and Hermes's own model routing.
- **Preservation plan (reviewed, nothing replaced):** Hermes `config.yaml` (OpenRouter glm-5.2 primary / oMLX Ornith fallback), launchd services, `API_SERVER_KEY` + LAN binding + PF anchor (Docker01-only — exactly the access needed), the n8n MCP front for ChatGPT, and the Hermes dashboard are all **unchanged**. Only Open WebUI's env changed (additive): `OPENAI_API_BASE_URLS`/`OPENAI_API_KEYS` (LiteLLM;Hermes), `AIOHTTP_CLIENT_TIMEOUT=1800` (matches Hermes gateway_timeout for long agent turns).
- Verified from inside the Open WebUI container: `/v1/models` → `hermes-agent`; non-stream "pong"; streaming SSE + `[DONE]`. Container recreated and healthy; no connection errors. UI picker confirmation lands on Jon's next visit.
- Out of scope (next milestone per the shared-knowledge ADR): Projects-knowledge indexing into Open WebUI; root `~/Projects` knowledge files (AGENTS.md/ABOUT-ME.md/PROJECT-KNOWLEDGE.md don't exist yet at the workspace root).
